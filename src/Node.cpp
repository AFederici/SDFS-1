...
void Node::masterDetection(){
	...
	if (port.size() > 0) {
		if (strcmp(potential.c_str(), nodeInformation.ip.c_str())){
			...
			if (alive_nodes >= REP && votes.size() > ((alive_nodes / 2) + 1)){
...
void Node::orderReplication(){
	for (auto &el : replicas_list){
		...
		if (get<1>(el.second).size() < REP) {
			...
			udpServent->sendMessage(get<0>(*it), get<1>(*it), msg.toString());
		}
	}
}
	...
	while(1){
		...
		if(cmd == "join"){
			...
			if (ss[0] == "put"){ node->handlePut(ss[1], ss[2]); }
				...
				if (get<1>(node->replicas_list[ss[1]]).size() == 0) { cout << "FILE DOESNT EXIST" << endl; continue; }
